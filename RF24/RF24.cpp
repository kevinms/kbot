...
{
  Serial.print("OBSERVE_TX=");Serial.print(value, HEX);
  Serial.print(" POLS_CNT=");Serial.print((value >> PLOS_CNT) & B1111, HEX);
  Serial.print(" ARC_CNT=");Serial.println((value >> ARC_CNT) & B1111, HEX);
}
...
void RF24::openReadingPipe(uint8_t child, uint8_t *address)
{
  ...
  if (child <= 6)
  {
    ...
    if ( child < 2 )
      write_register(RX_ADDR_P0+child, address, 5);
    else
      write_register(RX_ADDR_P0+child, address+4, 1);
    ...
    write_register(EN_AA,read_register(EN_AA) | _BV(ENAA_P0+child));
  }
}
...
void RF24::enableDynamicPayloads(void)
{
  ...
  Serial.println(read_register(FEATURE));
#endif
